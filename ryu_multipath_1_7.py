...
class ProjectController(app_manager.RyuApp):    
    ...
    ################################
    global ip_src, ip_dst, id_src, id_dst, dp, C, c, b, out_ports, PL, X, Y #, PL1_3, ipDFrame_src, arpDFrame_src, ipDFrame_dst, arpDFrame_dst 
    ...
    id_src = id_dst = ip_src = ip_dst = C = c = b = out_ports = PL = X = Y = 0#PL1_3 = 0
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.datapath_list = {}
        self.arp_table = {}        
        self.ip_list = {} # dic criada para armazenar o host e o ip { id: 'ip da maquina'}
        self.ev_port_Mod = {} #dic criado para armazenar o dp_id e a porta afetada { id: 'porta afetada'}
        self.switches = []
        self.all_switches = []
        self.hosts = {}
        ...
        self.eventos = []
        ##################################################
        #cliente para conexao com influxDB ip(docker) porta de conexao, e nome da base de dados
        self.client = InfluxDBClient(host='192.168.57.3', port=8086, database='sdn_icone')
    ######################################################
    ...
    def _switch_features_handler(self, ev):
        ...
        print colored('switch_features_handler is called','blue')
    ...
    def _packet_in_handler(self, ev):
        ...
        #link = get_link(self.topology_api_app, None)
        #links=[(link.src.dpid,link.dst.dpid,{'port':link.src.port_no}) for i in link]
        #print "ALL_link ", links
    ...
    #Funcao sera executada 2x vezes p/ deteccao de links caidos
    @set_ev_cls(event.EventPortModify, MAIN_DISPATCHER)
    def port_mod(self, ev):
        global ip_src, ip_dst, id_src, id_dst, X, Y
        
        #
        #tempo de captura + inferencia: inicio
        ...
        #processos abaixo servem para coletar informacoes
        #para deletar e criar linhas de fluxos
        #Esta "if" e executado 2x pq o link down e detectado
        #nas duas pontas
        if ev_list[2] == 'DOWN':
            #
            #tempo de captura + inferencia: fim
            start_time_1 = time.time()            
            t = start_time_1 - start_time
            time_1_2 = "{:.14f}".format(float(t)) #transforma de exponencial para float
            
            #tempo_fim - tempo_inicio
            #
            if (X == 0):#condicional de controle pq se nao ira salvar o tempo 2x
                print "tempo de captura de evento =", time_1_2 #'%.5f' % (time_1_2)
            ...
            o laco "for" abaixo separa o id_src e id_dst junto com ip_src
            e ip_dst e envia para a funcao flow_stats_reply_handler() para
            manipular as linhas de fluxos.
            '''
...
#===============================================================================================
    ...
    def flow_stats_reply_handler(self, ev):
        global id_src, id_dst, ip_src, ip_dst, ipDFrame_src, arpFrame_src, ipFrame_dst, arpFrame_dst
        ...
        for stat in sorted(ev.msg.body, key=attrgetter('match')):
            ...
            #MODIFICA LINHAS DE FLUXO PARA ARP
...
#
#        #print
#        #print "ev.msg.datapath ", ev.msg.datapath
#        #print
#        start_time = time.time()
...
#
#        #Nao pude usar este parte para recuperar o link pq ao iniciar a emulacao
#        #ele reconhece todos os links do ambiente atraves do OFPPR_ADD
...
#
#            for i in self.ip_list:                
#                if i == src_id: print self.ip_list[i]
#                if i == dst_id: print self.ip_list[i]
#                print
#            
...
#            pass
    ...
    ################################################################################################################
    ################################################################################################################
    #O CODIGO ABAIXO E RESPONSAVEL ESTATISTICA NA TELA
    ################################################################################################################
    ################################################################################################################
    ################################################################################################################
        ...
        if dpid == 1:
            for stat in sorted(body, key=attrgetter('port_no')):
                ...
                if stat.port_no == 2:
                    ...
                    #formato json para transferencia de dados para o influxdb banco sdn
                    #events = [
                    #        {
                    #            "measurement": "ovs1",
                    #            "tags": {
                    ...
                    #                },
                    #            "fields": {
                    #                "rec_band": float(result_rx_1_2),
                    #                "trans_band":float(result_1_2)
                    #                }
                    #            }
                    #        ]
                    #banco sdn_icone
                    ...
                    #cliente influx para escrever
                    self.client.write_points(events, time_precision=None)
                    #self.client.write_points(events)
                ###############################################################################
                #SELECIONA A PORTA 3
                if stat.port_no == 3:
                    self.logger.info('switch             '
                    ...
                    #                },
                    #            "fields": {
                    #                "rec_band": float(result_rx_1_3),
                    #                "trans_band":float(result_1_3)
                    #                }
                    #            }
                    #        ]
                    #databse sdn_icone
        ...
        if dpid == 2:
            for stat in sorted(body, key=attrgetter('port_no')):
                ...
                if stat.port_no == 2:
                    ...
                    #                },
                    #            "fields": {
                    #                "rec_band": float(result_rx_2_2),
                    #                "trans_band":float(result_2_2)
                    #                }
                    #            }
                    #        ]
                    #
                    ...
                    self.client.write_points(events, time_precision=None)
                    #####################################################
                    if c == 1 and dpid == 2: c += 1 #variavel de controle alcancada na porta 3 e adiciona + 1
                    try:
                        if (throughput2_3 > MAX_BAND*0.8) and c == 2:#
                            print '\033[1;31;47m Porta 2 Congestionada\033[1;m'# mensagem de porta
                            c += 1
                        elif (throughput2_3 < MAX_BAND*0.8) and c == 3: # quando a banda normalizar
                            c = 0
                            self.send_flow_mod(datapath, stat.port_no, self.ip_list[dpid])# e modifica o fluxo de volta para porta 3
                ...
                if stat.port_no == 3:
                    ...
                    #                },
                    #            "fields": {
                    #                "rec_band": float(result_rx_2_3),
                    #                "trans_band":float(result_2_3)
                    #                }
                    #            }
                    #        ]
                    #
                    ...
                    self.client.write_points(events, time_precision=None)
                    #####################################################
                    throughput2_3 = result_2_3 + result_rx_2_3
                    #####################################################
                    if (throughput2_3 > MAX_BAND*0.8) and c == 0 and dpid == 2:
                        self.send_flow_mod(datapath, stat.port_no, self.ip_list[dpid])
        ...
        if dpid == 3:
            for stat in sorted(body, key=attrgetter('port_no')):
                ...
                if stat.port_no == 3:
                    ...
                    result_3_3 = int(band_3_3/1048576)#Divide por 1 mega
                    ...
                    #                },
                    #            "fields": {
                    #                "rec_band": float(result_rx_3_3),
                    #                "trans_band":float(result_3_3)
                    #                }
                    #            }
                    #        ]
                    #database sdn_icone
                ...
                ###################################################################################
                if c == 1: c += 1 #variavel de controle alcancada na porta 3 e adiciona + 1
                if (throughput3_3 > MAX_BAND*0.8) and c == 2:# 
                    print '\033[1;31;47m Porta 3 Congestionada\033[1;m'# mensagem de porta 
                    c += 1 
                elif (throughput3_3 < MAX_BAND*0.8) and c == 3:# quando a banda normalizar 
                    c = 0                                    # zera a variável de controle
                    self.send_flow_mod(datapath, stat.port_no, self.ip_list[datapath.id])# e modifica o fluxo de volta para porta 2 
                    print '\033[1;34;47m Tráfego normalizado na porta ', stat.port_no,'\033[1;m'                
                ###################################################################################
                #SELECIONA A PORTA 2
                if stat.port_no == 2:
                    ...
                    result_3_2 = int(band_3_2/1048576) #Divide por 1Mb 
                    ...
                    #    
                    throughput3_2 = result_3_2 + result_rx_3_2
                    #
                    #database sdn
                    #events = [
                    #        {
                    ...
                    #                },
                    #            "fields": {
                    #                "rec_band": float(result_rx_3_2),
                    #                "trans_band":float(result_3_2)
                    #                }
                    #            }
                    #        ]
                    #
                    ...
                    if (throughput3_2 > MAX_BAND*0.8) and c == 0: #variavel c de controle
                        start_time_1 = time.time()
                        ...
                        #
                        self.send_flow_mod(datapath, stat.port_no, self.ip_list[datapath.id])
                        c += 1 #adiciona + 1 a variavel de controle
                    ...
                    #    print
                        ...
                        inference.close()
                        print "informações salvas"
                    elif (throughput3_2 >= MAX_BAND*0.6) and b == 0:
           ...
           for stat in sorted(body, key=attrgetter('port_no')):
               #SELECIONA A PORTA 1
                ...
                if stat.port_no == 2:
                    ...
                    #Calculo de banda para bytes recebidos (rx_bytes)
                    if rx_ini_4_2 == 0: rx_ini_4_2 = stat.rx_bytes # valor inicial bytes armazenado
                    #
                    ...
                    #                },
                    #            "fields": {
                    #                "rec_band": float(result_rx_4_2),
                    #                "trans_band":float(result_4_2)
                    #                }
                    #            }
                    #        ]
                    #
                ...
                if stat.port_no == 3:
                    ...
                    #                },
                    #            "fields": {
                    #                "rec_band": float(result_rx_4_3),
                    #                "trans_band":float(result_4_3)
                    #                }
                    #            }
                    #        ]
                    #
